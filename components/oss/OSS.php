...
class OSS extends Component
{
    ...
    /**
     * @var string 默认前缀（相当于文件夹）,注意要满足object命名要求，不能有前置斜杠
     */
    public $pathPrefix = '';
    ...
    {
        return $this->getClient()->uploadFile($this->bucket, $this->getPathWithPrefix($fileName), $filePath);
    }

    public function getPathWithPrefix($path){
    ...
    public function multiuploadFile($path, $filePath){
        $options = array(
            'fileUpload' => $filePath,
            'partSize' => 524288000
        );
        $result = $this->getClient()->multiuploadFile($this->bucket, $this->getPathWithPrefix($path), $filePath, $options);
        return $result;
    }
}
