...
class Uploadernew
{
     ...
     */
    public function __construct($file, $config, $type = "upload")
    {
        $this->config = $config;
        $this->type = $type;
        if ($type == "remote") {
    ...
    private function upFile($file)
    {
        ...
        if ($this->file['error']) {
            ...
            return;
        } else if (!file_exists($file['tempName'])) {
            $this->stateInfo = $this->getStateInfo("ERROR_TMP_FILE_NOT_FOUND");
            return;
        } else if (!is_uploaded_file($file['tempName'])) {
     ...
     * @uses 上传服务器文件
     */
    public function pdfImgUp(){

        include_once("ossnew.php");
        $serverpath = $this->fileField;
        $osspath = "/" . $this->config['pathFormat'];
        $ossboj = new OSSobjNew();
        $isok = $ossboj->upload_by_multi_part($serverpath, $osspath);
        if ($isok) {
            $this->stateInfo = "https://xykj-static2.oss-cn-hangzhou.aliyuncs.com".$osspath;//$this->stateMap[0];
